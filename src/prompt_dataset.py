...
class PromptDataset(Dataset):
    ...
    Attributes:
        ...
        repeat_gold (bool): Flag to repeat only the golden document num_documents_in_context times (no random documents).
        pad_gold (bool): Flag to replace non-golden documents with padding tokens matching their size, place golden document at end.
    """
    def __init__(
        self, 
        corpus: List[Dict],
        ...
        self.repeat_gold = repeat_gold
        self.pad_gold = pad_gold
        
        # Get pad token for padding - use whitespace instead of EOS token
        if self.pad_gold:
            # Use whitespace token instead of EOS token to avoid disruption
            # Whitespace is less disruptive than EOS token
            self.pad_token = " "
    ...
    def _validate_initialization_parameters(self):
        ...
        # When repeat_gold is True, gold_position is not needed (all positions are gold)
        if not self.repeat_gold:
    ...
    def prepare_documents_for_prompt(
        ...
        """
        # If repeat_gold is True, just repeat the gold document
        if self.repeat_gold:
            return self._repeat_gold_document(gold_document_idx)
        
        # Get indices and insert gold document
        ...
        # If pad_gold is True, replace non-gold documents with padding and place gold at end
        if self.pad_gold:
            return self._pad_gold_document(formatted_documents, document_indices, gold_document_idx, gold_position)
        
        return formatted_documents, document_indices


    def _repeat_gold_document(self, gold_document_idx: int) -> Tuple[List[str], List[int]]:
        """
        Repeats the golden document num_documents_in_context times.
        ...
        Returns:
            A tuple containing two lists:
            - The first list contains the formatted documents (gold document repeated).
            - The second list contains the indices of the included documents (all the same gold index).
        """
        # Get the gold document from corpus
        gold_doc_idx_int = int(gold_document_idx)
        
        # Handle full corpus vs subset corpus
        if self.full_to_subset_idx_map is None:
            doc_info = self.corpus[gold_doc_idx_int]
        else:
            # Map from full corpus index to subset index
            subset_idx = self.full_to_subset_idx_map[gold_doc_idx_int]
            doc_info = self.corpus[subset_idx]
        ...
        # Repeat the document num_documents_in_context times
        formatted_documents = [doc_str] * self.num_documents_in_context
        document_indices = [doc_idx] * self.num_documents_in_context
        
        return formatted_documents, document_indices

    def _pad_gold_document(
        self, 
        ...
        """
        Replaces non-golden documents with padding tokens matching their size, places golden document at end.
        
        Args:
            formatted_documents (List[str]): The original formatted documents (already retrieved).
            document_indices (List[int]): The indices of the original documents.
            gold_document_idx (int): The index of the gold document within the corpus.
            gold_position (int): The position of the gold document in the original list.
            
        Returns:
            A tuple containing two lists:
            - The first list contains formatted documents (padding for non-gold, gold at end).
            - The second list contains the indices of the included documents.
        """
        # Get the gold document string (it's already in formatted_documents at gold_position)
        gold_doc_str = formatted_documents[gold_position]
        gold_doc_idx = document_indices[gold_position]
        
        # Create padding documents matching the size of non-gold documents
        padded_documents = []
        padded_indices = []
        
        for i, doc_str in enumerate(formatted_documents):
            if i == gold_position:
                # Skip the gold document - we'll append it at the end
                continue
            else:
                # Non-gold document - create padding matching its size
                doc_tokens = self.tokenizer.tokenize(doc_str)
                num_tokens = len(doc_tokens)
                pad_str = self._create_padding_string(num_tokens)
                padded_documents.append(pad_str)
                padded_indices.append(document_indices[i])
        
        # Place the golden document at the end
        padded_documents.append(gold_doc_str)
        padded_indices.append(gold_doc_idx)
        
        return padded_documents, padded_indices

    def _create_padding_string(self, num_tokens: int) -> str:
        """
        Creates a padding string that matches the specified number of tokens.
        
        Args:
            num_tokens (int): The number of tokens the padding should match.
            
        Returns:
            str: A string containing padding tokens (no document format prefix).
        """
        # pad_token is already a string (handled in __init__)
        pad_token_text = self.pad_token
        
        # Estimate tokens per pad token
        pad_token_tokens = len(self.tokenizer.tokenize(pad_token_text))
        if pad_token_tokens == 0:
            pad_token_tokens = 1
        
        # Calculate how many pad tokens we need
        num_pad_repeats = max(1, num_tokens // pad_token_tokens)
        
        # Create padding string
        pad_content = pad_token_text * num_pad_repeats
        
        # Adjust if needed to match token count more closely
        pad_tokens = len(self.tokenizer.tokenize(pad_content))
        if pad_tokens < num_tokens:
            # Add more padding
            additional_tokens_needed = num_tokens - pad_tokens
            additional_pads = max(1, additional_tokens_needed // pad_token_tokens)
            pad_content += pad_token_text * additional_pads
    ...
    def build_qa_prompt(self, query: str, documents_str: str) -> str:
        ...
        #task_instruction = "You are given a question and you MUST respond by EXTRACTING the answer (max 5 tokens) from the provided document. If the document does not contain the answer, respond with NO-RES."

#        task_instruction = "You are given a question and you MUST respond with a short answer based on your internal knowledge. If you do not know the answer, please respond with NO-RES."
        #task_instruction = "You are given a question and you MUST respond by EXTRACTING the answer from one of the provided documents."
