...
def save_data_to_json(data_df: pd.DataFrame, directory: str, filename_prefix: str, overwrite: bool = False):
    """ Saves the given DataFrame to a JSON file. """
    data_path = os.path.join(directory, f'{filename_prefix}all.json')
    # Check if the file already exists
    if os.path.exists(data_path):
        if not overwrite:
            overwrite_input = input(f"File {data_path} already exists. Overwrite? (y/n): ")
            ...
            print(f"Overwriting existing file: {data_path}")
...
def get_classic_path(args):
    ...
    pad_gold_str = "_pad_gold" if getattr(args, 'pad_gold', False) else ""
    repeat_gold_str = "_repeat_gold" if getattr(args, 'repeat_gold', False) else ""

    filename_prefix = f'numdoc{args.num_doc}_gold_at{gold_pos}{rand_str}{answerless_str}{adore_str}{pad_gold_str}{repeat_gold_str}_info_'
...
def parse_arguments():
    ...
    parser.add_argument('--pad_gold', type=str2bool, help='Replace non-golden documents with padding tokens', default=False)
    parser.add_argument('--repeat_gold', type=str2bool, help='Repeat only the golden document', default=False)
    parser.add_argument('--overwrite', type=str2bool, help='Overwrite existing result files without prompting', default=True)
...
def main():
    ...
    data_path = save_data_to_json(data_df, directory, filename_prefix, overwrite=args.overwrite)
